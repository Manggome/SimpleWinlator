...
3. 오른쪽 위 **+** 를 누르고 둘 중 하나를 고릅니다
   - **exe 파일 하나 고르기** — 게임 폴더에서 `.exe` 를 직접 고릅니다
   - **폴더에서 게임 찾기** — 게임들이 들어 있는 폴더(예: `Games/`)를 고르면 하위 폴더까지 훑어서 실행 파일을 찾아 줍니다.
     설치 프로그램·런타임·크래시 리포터 같은 건 빼고, 게임 폴더마다 가장 그럴듯한 exe 하나에 미리 체크해 둡니다.
     이름은 게임 폴더 이름으로 붙습니다
...
- **폴더 스캔**으로 게임을 한꺼번에 추가
- 뒤로 가기는 항상 게임 목록으로 돌아옵니다
- **전체 한국어 번역** (설정 → 언어에서 한국어 선택 가능), 앱 이름 변경
- **앱 안에서 업데이트**: 하루에 한 번 GitHub Releases 를 확인하고 새 버전이 있으면 알려 줍니다.
  왼쪽 메뉴 **업데이트 확인** 으로 직접 확인할 수도 있습니다
...
1. `app/build.gradle` 의 `simpleVersion` 을 1 올립니다 (versionCode·versionName 이 같이 바뀝니다)
2. 커밋·푸시한 뒤 `v11.2-simple<번호>` 태그를 올립니다

```
git tag v11.2-simple3
git push origin v11.2-simple3
```

Actions 가 APK 를 빌드해 Release 에 붙이고, 설치된 앱들이 이걸 보고 업데이트를 알려 줍니다.
